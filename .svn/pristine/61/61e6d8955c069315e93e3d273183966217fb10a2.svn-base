			...
			<div class="container_12">
				...
				<div class="myForm">
					<form class="myfrm" method="post" action="Controller">
						<input type="hidden" name="com" value="register">
						<div class="grid_6">
							<div class="success_wrapper"></div>
							<c:choose>
								<c:when test="${exists!=null}"><p style="color: chocolate;"><fmt:message key='${exists}'/></p></c:when>
							</c:choose>
							<fieldset>
								<label class="name"> <input type="text" name="name"
									placeholder="<fmt:message key='NAME'/>" required> <br class="clear">

								</label> <label class="name"> <input type="text"
									name="second_name" placeholder="<fmt:message key='SECOND_NAME'/>" required>
									<br class="clear">

								</label> <label class="name"> <input type="email" name="email"
									placeholder="<fmt:message key='EMAIL'/>" required> <br class="clear">

								</label> <label class="name"> <input type="text" name="login"
								...
								<br/><br/>
       <% 
          ReCaptcha captcha = ReCaptchaFactory.newReCaptcha("6LefxSgTAAAAAExyvEcLOV-uT05ZJHEUZnhnlpnh", "6LefxSgTAAAAAKUeqs8u5nVSvFKasm2IYzEY-hno", false);
          out.print(captcha.createRecaptchaHtml(null, null));
        %>
							</fieldset>
							<button class="mybtn" type="submit"><fmt:message key='Send'/></button>
...
<c:set var="exists" value="${null}" scope="session"/>
</body>
</html>